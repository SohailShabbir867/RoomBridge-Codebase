...
import {
  RiMapPin2Line, RiHeart3Line, RiHeart3Fill,
  RiEyeLine, RiTimeLine, RiCheckboxCircleLine,
} from 'react-icons/ri';
...
const FALLBACK_IMG = 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=600&q=80';
...
const ListingCard = ({
  ...
  */
  ...
  const isOwner = listing.owner?._id === user?._id || listing.owner === user?._id;
      ...
      <Link to={`/listings/${listing._id}`}
        ...
        <div className="relative w-48 shrink-0 overflow-hidden">
          <img src={thumb} alt={listing.title}
               ...
               loading="lazy" />
          {showStatus && (
            <span className={`absolute top-2 left-2 text-xs font-medium px-2 py-0.5
        ...
        <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
          <div>
            <div className="flex items-start justify-between gap-2 mb-1">
              <h3 className="font-semibold text-primary line-clamp-2 leading-snug
                             group-hover:text-secondary transition-colors text-sm">
                {listing.title}
              </h3>
              {isAuthenticated && user?.role === 'seeker' && !isOwner && (
                <button onClick={handleSaveToggle} disabled={toggling}
    ...
    <Link to={`/listings/${listing._id}`}
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="text-xs font-medium bg-primary/90 backdrop-blur-sm text-white
        ...
        {listing.furnished && (
          <div className="absolute bottom-3 left-3">
            <span className="text-[10px] bg-white/90 backdrop-blur-sm text-primary font-medium
                             px-2 py-0.5 rounded-full flex items-center gap-1">
              <RiCheckboxCircleLine className="text-success text-xs" /> Furnished
            </span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col flex-1 p-4">
        {/* Title */}
        <h3 className="font-semibold text-primary line-clamp-2 leading-snug mb-1.5
                       group-hover:text-secondary transition-colors text-sm">
          {listing.title}
        </h3>
        ...
        {showOwner && listing.owner && (
          <p className="text-xs text-text-secondary mb-2">
            By: <span className="font-medium text-primary">{listing.owner.name}</span>
          </p>
        )}
        ...
        <div className="flex items-end justify-between mt-2">
          ...
          {listing.availableFrom && (
            <span className="text-xs text-text-secondary">
              From {new Date(listing.availableFrom).toLocaleDateString('en-PK', { month: 'short', day: 'numeric' })}
            </span>
          )}
        </div>
      </div>
