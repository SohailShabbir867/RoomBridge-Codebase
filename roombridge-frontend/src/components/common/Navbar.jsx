...
import {
  ...
  RiUserLine, RiDashboardLine, RiLogoutBoxLine,
  RiArrowDownSLine, RiHome4Line, RiInformationLine, RiAddCircleLine,
  RiMessageLine,
} from 'react-icons/ri';
...
const Navbar = () => {
  ...
  const handleLogout = useCallback(async () => {
    ...
    /*
      BUG FIX: store.js rootReducer intercepts logout action and resets
  ...
  const messagesLink = user?.role === 'owner'
    ? '/owner/messages'
    : '/seeker/messages';

  /*
    BUG FIX: Nav links are defined inside the component so the "Post a Room"
  ...
  */
  const NAV_LINKS = [
    ...
    ...(isAuthenticated && user?.role === 'owner'
      ? [{ to: '/owner/listings/create', label: 'Post a Room', icon: RiAddCircleLine }]
  ...
  return (
    ...
    >
            ...
            <>
              ...
              <div className="relative" ref={profileRef}>
                ...
                >
                  ...
                  <span className="text-sm font-medium text-primary max-w-[96px] truncate">
                    {user.name}
                  </span>
                  <RiArrowDownSLine
                ...
                {/* Dropdown */}
                {profileOpen && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-card
                    ...
                    <div className="p-1" role="none">
                      ...
                      >
                        <RiDashboardLine className="text-base text-secondary" /> Dashboard
                      </Link>
                      ...
                      {user.role === 'seeker' && (
                        <Link
                          ...
                          className="flex items-center gap-2 w-full px-3 py-2 text-sm
                                     text-primary rounded-lg hover:bg-background transition-colors"
                        >
                          <RiUserLine className="text-base text-secondary" /> My Profile
                        </Link>
                      )}
                      ...
                      >
                        ...
                        <span>Messages</span>
                        {unreadCount > 0 && (
                          <span className="ml-auto bg-error text-white text-[10px]
      ...
      {/* ── Mobile drawer ──────────────────────────────────────── */}
      {/*
        BUG FIX: was using conditional rendering with {menuOpen && <div>}.
              ...
              {isAuthenticated && user ? (
                ...
                <>
                  <Link
                    to="/login"
                    onClick={() => setMenuOpen(false)}
                    className="block text-center py-2.5 text-sm font-semibold text-primary
