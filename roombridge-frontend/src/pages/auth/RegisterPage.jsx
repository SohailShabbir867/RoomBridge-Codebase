...
import {
  ...
  RiLockLine, RiUserLine, RiPhoneLine, RiMapPin2Line,
  RiUserAddLine, RiHome4Line, RiKeyLine,
} from 'react-icons/ri';
...
/*
  BUG FIX: index 0 must have a value so the label/color doesn't show
...
/*
  BUG FIX: LISTING_TYPES in constants.js uses 'single'/'shared'/'apartment'
...
const RegisterPage = () => {
  ...
  /* BUG FIX: validate role from URL — only accept 'owner' or 'seeker' */
  const rawRole        = searchParams.get('role') || '';
  const preselectedRole = VALID_ROLES.includes(rawRole) ? rawRole : '';

  const [form, setForm] = useState({
    name: '', email: '', password: '', confirmPassword: '',
    phone: '', city: '', role: preselectedRole, terms: false,
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      /*
        BUG FIX: read from the axios error shape, not err.message directly.
      */
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-5/12 xl:w-1/2 bg-primary flex-col justify-between p-12">
        ...
        <div>
          <h1 className="text-4xl xl:text-5xl font-bold text-white leading-tight mb-4">
            Join <span className="text-accent">Thousands</span><br/>of Happy Members
          </h1>
          <p className="text-white/65 text-lg mb-10">
            Whether you have a room to offer or need one — we've got you covered.
          </p>
          {[
            { icon: RiHome4Line,    title: 'Room Owners',  desc: 'List your room and get verified tenants fast.'  },
            { icon: RiKeyLine,      title: 'Room Seekers', desc: 'Find rooms that match your budget & lifestyle.' },
            { icon: RiBuildingLine, title: 'Smart Match',  desc: 'AI-powered roommate compatibility scoring.'     },
        ...
        <p className="text-white/30 text-xs">© {new Date().getFullYear()} RoomBridge Pakistan</p>
      </div>
      ...
      <div className="flex-1 flex items-start justify-center px-6 py-10 bg-background overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          <h2 className="text-3xl font-bold text-primary mb-1">Create your account</h2>
          <p className="text-text-secondary mb-7">Join RoomBridge — it's free!</p>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3" role="radiogroup" aria-label="Account type">
                {[
                  { value: 'seeker', icon: RiKeyLine,   label: 'Looking for a Room', sub: 'Seeker' },
                  { value: 'owner',  icon: RiHome4Line, label: 'Listing a Room',     sub: 'Owner'  },
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="reg-name" name="name" type="text" value={form.name}
                       onChange={handleChange} placeholder="Muhammad Ali"
                       autoComplete="name"
            ...
            <div>
              ...
              {form.password && (
                <div className="mt-2" aria-label={`Password strength: ${STRENGTH_LABELS[pwStrength] || 'too short'}`}>
                  ...
                  {/*
                    BUG FIX: pwStrength 0 means password exists but is empty-length — impossible
                    since we only render when form.password is truthy. Score 1–4 maps to labels.
                    Guard with || '' to prevent "undefined password".
                  */}
                  <p className={`text-xs mt-1 font-medium ${STRENGTH_TEXT[pwStrength] || 'text-text-secondary'}`}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                {errors.phone && <p className="error-msg text-[11px]">{errors.phone}</p>}
              </div>
              <div>
            ...
            <div>
              <label className="flex items-start gap-2.5 cursor-pointer select-none">
                <input name="terms" type="checkbox" checked={form.terms}
