...
import {
  RiArrowLeftLine, RiFlagLine, RiLoader4Line,
  RiUserLine, RiHome4Line, RiCheckLine, RiCloseLine,
  RiRefreshLine, RiTimeLine, RiEyeLine,
} from 'react-icons/ri';
...
const Avatar = ({ user, size = 'sm' }) => {
  ...
  if (!user) return (
    <div className={`${sz} rounded-full bg-border flex items-center justify-center shrink-0`}>
      ...
      <div className="bg-white rounded-card shadow-hover w-full max-w-lg p-6">
            ...
            {['reviewed', 'resolved', 'dismissed'].map(s => (
              <button key={s} type="button" onClick={() => setStatus(s)}
                      className={`flex-1 py-2 rounded-btn text-sm font-medium capitalize border transition-all
                                  ${status === s
                                    ? s === 'resolved'  ? 'bg-success text-white border-success'
                                    : s === 'reviewed'  ? 'bg-primary text-white border-primary'
        ...
        <div className="mb-5">
          ...
          <textarea value={adminNote} onChange={e => setAdminNote(e.target.value)}
                    rows={3} placeholder="Internal note or action taken…"
                    className="input resize-none" maxLength={500} />
          <p className="text-xs text-text-secondary text-right mt-0.5">{adminNote.length}/500</p>
        </div>

        <div className="flex gap-3">
          <button onClick={onClose} className="btn-secondary flex-1 justify-center">Cancel</button>
          <button onClick={() => onSubmit({ status, adminNote })}
                  disabled={updating}
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-20 bg-white border-b border-border px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-bold text-primary">User Reports</h1>
            <p className="text-text-secondary text-xs">
              {total} total · {pendingCount} pending action
            </p>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        {loading && reports.length === 0 ? (
          ...
          <div className="text-center py-20 bg-white rounded-card border border-border shadow-card">
            ...
            <p className="text-primary font-semibold text-lg">No reports found</p>
          ...
          <div className="bg-white rounded-card border border-border shadow-card overflow-hidden">
            ...
            <div className="grid grid-cols-[1fr_1fr_120px_100px_120px] gap-0 border-b border-border bg-background px-4 py-3">
              <div className="text-xs font-semibold text-text-secondary uppercase">Reporter</div>
              <div className="text-xs font-semibold text-text-secondary uppercase">Reported</div>
              <div className="text-xs font-semibold text-text-secondary uppercase">Reason</div>
              <div className="text-xs font-semibold text-text-secondary uppercase">Status</div>
              <div className="text-xs font-semibold text-text-secondary uppercase text-right">Actions</div>
            </div>
            ...
            <div className="divide-y divide-border">
              {reports.map(r => {
                ...
                return (
                  <div key={r._id} className="hover:bg-background/30 transition-colors">
                    {/* Main row */}
                    <div className="grid grid-cols-[1fr_1fr_120px_100px_120px] gap-0 items-center px-4 py-4">
                      ...
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-text-secondary truncate">{r.reporter?.email}</p>
                        </div>
                      </div>
                          ...
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-text-secondary">
                                {r.reportedListing.city} · PKR {(r.reportedListing.rent || 0).toLocaleString()}
                              </p>
                            </div>
                          </div>
                        ) : (
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        {!canAct && (
                          <span className="text-xs text-text-secondary italic capitalize">{r.status}</span>
                        )}
                      </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-primary bg-white border border-border rounded-input p-3">
                              {r.description || <span className="italic text-text-secondary">No description provided.</span>}
                            </p>
                          </div>
                          {/* Meta */}
                          <div className="space-y-2">
                            <div>
                              <p className="text-xs font-semibold text-text-secondary uppercase mb-1">Timeline</p>
                              <p className="text-xs text-text-secondary flex items-center gap-1">
                                <RiTimeLine /> Reported: {new Date(r.createdAt).toLocaleString()}
                              </p>
                              {r.resolvedAt && (
                            ...
                            {r.adminNote && (
                              <div>
                                <p className="text-xs font-semibold text-text-secondary uppercase mb-1">Admin Note</p>
                                <p className="text-sm text-primary bg-white border border-border rounded-input p-3">
                                  {r.adminNote}
                                </p>
                              </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-border bg-background/50">
                <p className="text-xs text-text-secondary">Page {page} of {totalPages} · {total} reports</p>
                <div className="flex gap-2">
                  <button onClick={() => fetchReports({ page: page - 1 })} disabled={page <= 1 || loading}
                  ...
                  <button onClick={() => fetchReports({ page: page + 1 })} disabled={page >= totalPages || loading}
                          className="px-3 py-1.5 rounded-btn text-sm border border-border text-text-secondary
                                     hover:text-primary disabled:opacity-40 transition-colors">
                    Next →
                  </button>
