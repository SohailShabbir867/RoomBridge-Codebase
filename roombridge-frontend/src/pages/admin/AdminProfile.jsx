...
const AdminProfile = () => {
  ...
  return (
    <RoleDashboardLayout role="admin" title="My Profile" subtitle="Manage your account details">
      <div className="max-w-2xl mx-auto space-y-5">

        {/* ── Profile card ─────────────────────────────────────── */}
        <div className="bg-white rounded-2xl border shadow-sm p-6" style={{ borderColor: "#E8E2D9" }}>
              ...
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    <RiShieldCheckLine className="text-xs" style={{ color: ACC }} />
                ...
                {!editing && (
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-xl text-white hover:opacity-90 transition-all"
                    style={{ backgroundColor: BTN }}
                  >
                    <RiEditLine /> Edit
                  </button>
          ...
          {/* Edit form */}
          {editing && (
            <form onSubmit={handleSaveProfile} className="pt-5 border-t space-y-4" style={{ borderColor: "#F3EFE9" }}>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Full Name</label>
                <input name="name" value={form.name} onChange={handleChange} className={inputCls} style={inputStyle} placeholder="Your name" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Phone</label>
                  <input name="phone" value={form.phone} onChange={handleChange} className={inputCls} style={inputStyle} placeholder="03xxxxxxxxx" />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">City</label>
                  <select name="city" value={form.city} onChange={handleChange} className={inputCls} style={inputStyle}>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setEditing(false)}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border text-sm font-semibold text-gray-500 hover:bg-gray-50 transition-all"
        ...
        <div className="bg-white rounded-2xl border shadow-sm p-6" style={{ borderColor: "#E8E2D9" }}>
          ...
          {!showPwSection && <p className="text-xs text-gray-400 mt-1">Keep your account secure with a strong password.</p>}

          {showPwSection && (
            <form onSubmit={handleChangePassword} className="mt-5 pt-5 border-t space-y-4" style={{ borderColor: "#F3EFE9" }}>
              {[
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    style={inputStyle}
                  />
                  {pwErrors[key] && <p className="text-red-500 text-xs mt-1 font-medium">{pwErrors[key]}</p>}
                </div>
              ))}
              <button type="submit" disabled={savingPw}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-white text-sm font-bold hover:opacity-90 disabled:opacity-60 transition-all"
                style={{ backgroundColor: DK }}>
        ...
        <div className="bg-white rounded-2xl border shadow-sm p-6" style={{ borderColor: "#E8E2D9" }}>
          ...
          <button onClick={handleLogout}
            className="flex items-center gap-2 text-xs font-bold px-4 py-2.5 rounded-xl border text-red-500 hover:bg-red-50 transition-all"
