...
import {
  ...
  RiHome4Line, RiMapPin2Line, RiMoneyDollarCircleLine,
...
/*
  BUG FIX: Room type values must match the Listing.model.js enum exactly:
...
const CreateListing = () => {
  ...
  /*
    BUG FIX: The old code used field name 'features' and sent:
  ...
  */
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      /*
        BUG FIX: amenities (formerly 'features') must be appended as individual
      ...
      */
      ...
      /* BUG FIX: err.message is undefined on axios errors */
  ...
  const resetForm = () => {
    ...
    setForm({
      ...
      roomType:'', genderPreference:'any', availableFrom:'', furnished:false, amenities:[],
    });
    setPhotos([]);
    setPreviews([]);
    setErrors({});
  ...
  if (success) return (
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      <div className="bg-white rounded-card shadow-card border border-border p-10 text-center max-w-md w-full">
        ...
        <h2 className="text-2xl font-bold text-primary mb-2">Submitted for Review!</h2>
        <p className="text-text-secondary text-sm mb-6">
          Your listing is pending admin approval. You'll be notified once it's live.
        </p>
        <div className="flex gap-3 justify-center">
          <Link to="/owner/listings" className="btn-secondary">My Listings</Link>
          <button onClick={resetForm} className="btn-primary">Post Another</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-20 bg-white border-b border-border px-6 py-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-text-secondary text-xs">Fill in details to list your room</p>
        </div>
      </header>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <form onSubmit={handleSubmit} noValidate className="space-y-8">
          ...
          <div className="bg-white rounded-card border border-border shadow-card p-6">
            ...
            <p className="text-xs text-text-secondary mb-4">Upload up to 6 photos. First photo will be the cover.</p>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 mb-3">
              {previews.map((src, i) => (
                <div key={i} className="relative aspect-square rounded-lg overflow-hidden border border-border">
                  ...
                  {i === 0 && <span className="absolute bottom-1 left-1 text-[9px] bg-primary text-white px-1 rounded">Cover</span>}
                </div>
              ))}
              {photos.length < 6 && (
          ...
          <div className="bg-white rounded-card border border-border shadow-card p-6 space-y-4">
            ...
            <div>
              <label className="label" htmlFor="cl-title">Listing Title *</label>
              <input id="cl-title" name="title" value={form.title} onChange={handleChange}
                     placeholder="e.g. Cozy Single Room in DHA Phase 6"
                     className={`input ${errors.title ? 'input-error' : ''}`} />
              <div className="flex items-center justify-between mt-1">
                {errors.title && <p className="error-msg">{errors.title}</p>}
                <span className="text-xs text-text-secondary ml-auto">{form.title.length}/100</span>
              </div>
            </div>

            <div>
              <label className="label" htmlFor="cl-desc">Description *</label>
              <textarea id="cl-desc" name="description" value={form.description} onChange={handleChange}
                        rows={5} placeholder="Describe your room in detail — size, floor, nearby facilities..."
                        className={`input resize-none ${errors.description ? 'input-error' : ''}`} />
              <div className="flex items-center justify-between mt-1">
                {errors.description && <p className="error-msg">{errors.description}</p>}
                <span className="text-xs text-text-secondary ml-auto">{form.description.length}/2000</span>
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="label" htmlFor="cl-roomType">Room Type *</label>
                {/*
                  BUG FIX: Values now match the backend Listing model enum:
                  single_room | shared_room | full_apartment | hostel
                */}
                <select id="cl-roomType" name="roomType" value={form.roomType} onChange={handleChange}
              ...
              <div>
                <label className="label" htmlFor="cl-gender">Gender Preference</label>
                <select id="cl-gender" name="genderPreference" value={form.genderPreference}
            ...
            <label className="flex items-center gap-3 p-3 bg-background rounded-input border border-border cursor-pointer select-none">
              <input type="checkbox" id="cl-furnished" name="furnished" checked={form.furnished}
                     onChange={handleChange} className="accent-primary w-4 h-4" />
          ...
          <div className="bg-white rounded-card border border-border shadow-card p-6 space-y-4">
            <h2 className="font-semibold text-primary flex items-center gap-2">
              <RiMoneyDollarCircleLine className="text-secondary" /> Pricing & Availability
            </h2>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="label" htmlFor="cl-rent">Monthly Rent (PKR) *</label>
                <div className="relative">
                  <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm text-text-secondary font-medium pointer-events-none">₨</span>
                  <input id="cl-rent" name="rent" type="number" min="1000" max="500000"
              ...
              <div>
                <label className="label" htmlFor="cl-available">Available From *</label>
                <input id="cl-available" name="availableFrom" type="date"
                       min={new Date().toISOString().split('T')[0]}
                       value={form.availableFrom} onChange={handleChange}
                       className={`input ${errors.availableFrom ? 'input-error' : ''}`} />
                {errors.availableFrom && <p className="error-msg">{errors.availableFrom}</p>}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-card border border-border shadow-card p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="label" htmlFor="cl-city">City *</label>
                <select id="cl-city" name="city" value={form.city} onChange={handleChange}
              ...
              <div>
                <label className="label" htmlFor="cl-area">Area / Sector</label>
                <input id="cl-area" name="area" value={form.area} onChange={handleChange}
                       placeholder="e.g. DHA Phase 6"
                       className="input" />
              </div>
            </div>
            <div>
              <label className="label" htmlFor="cl-address">Full Address *</label>
              <input id="cl-address" name="address" value={form.address} onChange={handleChange}
          ...
          {/* Amenities
            BUG FIX: Was called 'features' and sent as JSON objects [{name, available}].
          ...
          */}
          ...
          <div className="flex gap-3">
            <Link to="/owner/dashboard" className="btn-secondary flex-1 justify-center">
              Cancel
            </Link>
