...
import {
  RiMapPin2Line, RiStarFill, RiHeart3Line, RiHeart3Fill,
  RiShareLine, RiCheckLine, RiPhoneLine,
  RiMessage3Line, RiWifiLine, RiTempColdLine, RiCarLine,
  RiRestaurantLine, RiShirtLine, RiShieldLine, RiPlugLine,
  RiGroupLine, RiCalendarLine, RiArrowRightSLine,
  RiArrowLeftSLine, RiArrowRightLine, RiLoader4Line,
  RiAlertLine,
} from 'react-icons/ri';
...
const BookingForm = ({ listing, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!canBook) {
      toast.error(isOwner ? "You can't book your own listing." : 'Only seekers can send booking requests.');
      return;
    }
    if (!validate()) return;
    try {
  ...
  if (user?.role !== 'seeker') {
    return (
      <div className="border border-border rounded-input p-4 text-center">
        <p className="text-text-secondary text-sm">Only seekers can book rooms.</p>
      </div>
    );
  }
  ...
  if (sent) {
    return (
      <div className="bg-success/10 border border-success/30 rounded-input p-4 text-center">
        ...
        <p className="text-success text-sm font-medium">Booking request sent!</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        ...
        <div className="relative">
          ...
          <input
            ...
            onChange={(e) => { setBooking((b) => ({ ...b, moveInDate: e.target.value })); setErrors({}); }}
      ...
      <div>
        <label className="label" htmlFor="booking-message">Message to Owner</label>
        <textarea
          id="booking-message"
          value={booking.message}
          onChange={(e) => { setBooking((b) => ({ ...b, message: e.target.value })); setErrors(er => ({...er, message: undefined})); }}
          rows={3}
...
const ListingDetailPage = () => {
  ...
  const loadListing = useCallback(async () => {
    try {
      ...
      /*
        BUG FIX: was using hardcoded MOCK data — never called the API.
  ...
  const prevImg = () => setActiveImg((i) => (i === 0 ? photos.length - 1 : i - 1));
  const nextImg = () => setActiveImg((i) => (i === photos.length - 1 ? 0 : i + 1));
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-text-secondary mb-6">{error || 'This listing is no longer available.'}</p>
  ...
  /*
    BUG FIX: listing.price doesn't exist on the model — the field is 'rent'.
    BUG FIX: listing.type doesn't exist on the model — the field is 'roomType'.
  */
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            >
              {saveLoading
                ...
                : saved ? <RiHeart3Fill /> : <RiHeart3Line />
              }
              {saved ? 'Saved' : 'Save'}
            </button>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {photos.length > 0 ? (
              ...
              <div className="w-full h-72 bg-background rounded-card border border-border
                              flex items-center justify-center">
                <p className="text-text-secondary text-sm">No photos available</p>
              </div>
            )}

            {/* Description */}
            <div className="bg-white rounded-card border border-border p-6">
              <h2 className="text-lg font-bold text-primary mb-3">About this Room</h2>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-5 pt-5 border-t border-border">
                {[
                  ...
                  listing.genderPreference && {
                    label: 'Preferred',
                    value: listing.genderPreference === 'any' ? 'Any gender' : `${listing.genderPreference} only`,
                  },
                  listing.availableFrom && {
                    label: 'Available From',
                    value: new Date(listing.availableFrom).toLocaleDateString('en-PK', { month: 'short', day: 'numeric', year: 'numeric' }),
                  },
              ...
              <div className="bg-white rounded-card border border-border p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {amenities.map((a) => {
                    ...
                    return (
                      <div key={a} className="flex items-center gap-2.5 p-3 bg-background rounded-lg">
            ...
            {rules.length > 0 && (
              <div className="bg-white rounded-card border border-border p-6">
                <h2 className="text-lg font-bold text-primary mb-4">House Rules</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {rules.map((rule) => (
                    <div key={rule} className="flex items-center gap-2 text-sm text-text-secondary">
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                {listing.averageRating > 0 && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-bold text-primary">{Number(listing.averageRating).toFixed(1)}</span>
                  </div>
                )}
              </div>
                ...
                <div className="space-y-4">
                  {reviewList.map((r, idx) => (
                    <div key={r._id || idx}
                      ...
                      <p className="text-text-secondary text-sm ml-10">{r.comment || r.text}</p>
                    </div>
                  ))}
                </div>
              ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-card border border-border p-6 sticky top-36">
              <div className="flex items-end gap-2 mb-1">
                {/* BUG FIX: was listing.price — correct field is listing.rent */}
              ...
              <div className="border-t border-border mt-4 pt-4 space-y-2">
                {/* Message owner */}
                <Link
                  to={isAuthenticated ? `/seeker/messages?owner=${listing.owner?._id || ''}` : '/login'}
                  state={!isAuthenticated ? { from: `/listings/${id}` } : undefined}
                ...
                {ownerPhone && (
                  <a href={`tel:${ownerPhone}`}
                     className="w-full flex items-center justify-center gap-2
                                ...
                                py-2.5 rounded-btn hover:border-primary hover:text-primary
                                transition-all duration-200">
                    <RiPhoneLine /> {ownerPhone}
                  </a>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-card border border-border p-5">
              <h3 className="font-semibold text-primary mb-3 text-sm">About the Owner</h3>
              <div className="flex items-center gap-3 mb-4">
                {listing.owner?.profilePhoto?.url ? (
                  <img src={listing.owner.profilePhoto.url} alt={ownerName}
                ...
                <div>
                  <p className="font-semibold text-primary text-sm">{ownerName}</p>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Listings',      value: listing.owner?.listingCount ?? '–' },
                  { label: 'Response Rate', value: listing.owner?.responseRate ? `${listing.owner.responseRate}%` : '–' },
                ].map(({ label, value }) => (
