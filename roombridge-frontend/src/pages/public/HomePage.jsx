...
import {
  RiSearchLine, RiMapPin2Line, RiBuilding4Line,
  RiGroupLine, RiShieldCheckLine, RiStarFill,
  RiArrowRightLine, RiHome4Line, RiCheckLine,
  RiArrowRightSLine, RiLoader4Line,
} from 'react-icons/ri';
...
const HOW_IT_WORKS = [
  { step: '01', icon: RiSearchLine,       title: 'Search Rooms',    desc: 'Browse thousands of verified rooms filtered by city, budget, and amenities.' },
  { step: '02', icon: RiGroupLine,        title: 'Match Roommates', desc: 'Our smart algorithm matches you with compatible roommates based on lifestyle preferences.' },
  { step: '03', icon: RiHome4Line,        title: 'Connect & Book',  desc: 'Chat with owners, schedule visits, and confirm your booking — all in one place.' },
  { step: '04', icon: RiCheckLine,        title: 'Move In!',        desc: 'Move into your verified room with confidence. RoomBridge has you covered.' },
];

const FEATURED_CITIES = [
  { name: 'Karachi',    rooms: '3,240', img: 'https://images.unsplash.com/photo-1564507592333-c60657eea523?w=400&q=80' },
  { name: 'Lahore',     rooms: '2,890', img: 'https://images.unsplash.com/photo-1567157577867-05ccb1388e66?w=400&q=80' },
  { name: 'Islamabad',  rooms: '1,540', img: 'https://images.unsplash.com/photo-1583417319070-4a69db38a482?w=400&q=80' },
  { name: 'Rawalpindi', rooms: '980',   img: 'https://images.unsplash.com/photo-1512453979798-5ea266f8880c?w=400&q=80' },
  { name: 'Peshawar',   rooms: '620',   img: 'https://images.unsplash.com/photo-1591604466107-ec97de577aff?w=400&q=80' },
  { name: 'Multan',     rooms: '450',   img: 'https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=400&q=80' },
];

const TESTIMONIALS = [
  { name: 'Ahmed Raza', city: 'Karachi',  role: 'Seeker', rating: 5, text: 'Found my perfect room in 2 days! The roommate matching feature is genius.' },
  { name: 'Sara Khan',  city: 'Lahore',   role: 'Owner',  rating: 5, text: 'Listed my room and got 5 serious inquiries the same day. Amazing platform!' },
  { name: 'Bilal Malik',city: 'Islamabad',role: 'Seeker', rating: 5, text: 'The compatibility score really works. My roommate and I get along perfectly!' },
];

/*
  Budget range → minRent / maxRent query params.
  BUG FIX: HomePage was sending ?budget=0-10000 which the backend doesn't
    ...
    <Link to={`/listings/${listing._id}`}
      ...
      <div className="relative overflow-hidden">
        <img src={photo} alt={listing.title}
             onError={(e) => { e.target.src = 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?w=500&q=80'; }}
             className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-500" />
        <div className="absolute top-3 left-3 bg-primary/90 text-white text-xs font-medium
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        <div className="mt-auto flex items-center justify-between">
          ...
          <span className="text-xs font-semibold text-secondary border border-secondary/30
...
const HomePage = () => {
  ...
  const loadFeatured = useCallback(async () => {
    try {
      ...
      const res = await listingService.getListings({ limit: 6, sortBy: 'newest' });
      /*
        BUG FIX: backend returns { success, listings, pagination }
        not { data }. Read res.listings directly.
      */
      setFeaturedListings(Array.isArray(res.data) ? res.data : res.listings ?? []);
    } catch {
  ...
  const handleSearch = (e) => {
    ...
    /*
      BUG FIX: was sending ?type= but backend filter param is roomType.
      Now correctly sends roomType.
    */
    if (search.roomType) params.set('roomType', search.roomType);
    /*
      BUG FIX: was sending ?budget=0-10000 (string range) — backend
  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="relative bg-primary overflow-hidden">
        ...
        <div className="absolute -top-32 -right-32 w-96 h-96 bg-accent/10 rounded-full blur-3xl" aria-hidden="true" />
        <div className="absolute -bottom-20 -left-20 w-80 h-80 bg-secondary/20 rounded-full blur-3xl" aria-hidden="true" />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32">
          <div className="max-w-3xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 bg-white/10 text-accent px-4 py-1.5
                            rounded-full text-sm font-medium mb-6 border border-white/20">
              <RiStarFill className="text-xs" /> Pakistan's #1 Room Rental Platform
            </div>

            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white leading-tight mb-5">
              Find Your <span className="text-accent">Perfect Room</span><br />
              <span className="text-white/85">and the Right Roommate</span>
            </h1>
            <p className="text-white/70 text-lg md:text-xl mb-10 max-w-2xl mx-auto">
              Browse thousands of verified rooms across Pakistan. Smart roommate matching
              based on your lifestyle, budget, and preferences.
            </p>

            {/* Search box */}
            <form onSubmit={handleSearch}
                  className="bg-white rounded-2xl shadow-hover p-3
                             flex flex-col sm:flex-row gap-2 max-w-2xl mx-auto">
              {/* City */}
              <div className="flex-1 relative">
                ...
                <select
                  ...
                  className="w-full pl-9 pr-3 py-3 text-sm text-primary bg-transparent
              ...
              {/* Type — BUG FIX: was binding to search.type, now search.roomType */}
              <div className="flex-1 relative">
                ...
                <select
                  ...
                  onChange={(e) => setSearch((s) => ({ ...s, roomType: e.target.value }))}
                  className="w-full pl-9 pr-3 py-3 text-sm text-primary bg-transparent
                             outline-none sm:border-r border-border appearance-none cursor-pointer"
                >
                  <option value="">Any Type</option>
                  {/* BUG FIX: values now match backend Listing.model.js enum */}
              ...
              {/* Budget — BUG FIX: binds to search.budget (the range string).
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-5">
              {['Karachi', 'Lahore', 'Islamabad', 'Rawalpindi', 'Peshawar'].map((c) => (
                <button key={c}
                        ...
                        className="text-xs font-medium text-white/70 border border-white/20
                                   px-3 py-1.5 rounded-full hover:bg-white/10 hover:text-white
                                   transition-all duration-200">
                  {c}
                </button>
              ))}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {featuredLoading ? (
            <div className="flex items-center justify-center py-20">
              <RiLoader4Line className="animate-spin text-4xl text-primary" aria-label="Loading listings" />
            </div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-text-secondary">No listings yet. Be the first to post!</p>
              <Link to="/register" className="btn-primary mt-4 inline-flex gap-2">
                Post a Room
              </Link>
            </div>
          )}
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
            {FEATURED_CITIES.map(({ name, rooms, img }) => (
              <Link key={name} to={`/listings?city=${name}`}
                ...
                <div className="absolute top-3 right-3 bg-white/10 backdrop-blur-sm
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-background">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-4xl font-bold text-primary mb-3">How RoomBridge Works</h2>
            <p className="text-text-secondary text-lg">Find your room in 4 simple steps</p>
          </div>
            ...
            {HOW_IT_WORKS.map(({ step, icon: Icon, title, desc }, i) => (
              <div key={step} className="relative text-center group">
                ...
                {i < HOW_IT_WORKS.length - 1 && (
                  <div className="hidden lg:block absolute top-8 left-1/2 w-full h-px
                                  bg-gradient-to-r from-accent to-transparent" aria-hidden="true" />
                )}
                <div className="relative inline-flex flex-col items-center">
                  ...
                  <span className="text-xs font-bold text-accent tracking-widest mb-2">{step}</span>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-7xl mx-auto">
          <div className="bg-primary rounded-2xl overflow-hidden relative">
            <div className="absolute -top-10 -right-10 w-64 h-64 bg-accent/10 rounded-full blur-2xl" aria-hidden="true" />
            <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-0">
              <div className="p-10 lg:p-14">
                <div className="inline-flex items-center gap-2 bg-accent/20 text-accent
                ...
                <h2 className="text-3xl md:text-4xl font-bold text-white mb-4 leading-tight">
                  Find Roommates Who <span className="text-accent">Actually Match</span> Your Lifestyle
                </h2>
                <p className="text-white/70 text-base mb-8 leading-relaxed">
                  Answer a few questions about your sleep schedule, cleanliness habits, and social preferences.
                  Our AI engine will find your top matches — with a compatibility score.
                </p>
              ...
              <div className="relative hidden lg:flex items-center justify-center p-10">
                <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 w-72 border border-white/20">
                  <p className="text-white/70 text-sm mb-3">Compatibility Score</p>
                  <div className="flex items-end gap-2 mb-4">
                    <span className="text-5xl font-bold text-white">87</span>
                    <span className="text-accent text-xl font-semibold mb-1">/ 100</span>
                  </div>
                  ...
                  {[
                    ...
                    <div key={label} className="flex items-center gap-3 mb-2.5">
                      <p className="text-white/65 text-xs w-28 shrink-0">{label}</p>
                      <div className="flex-1 h-1.5 bg-white/20 rounded-full">
                        <div className="h-1.5 bg-accent/70 rounded-full" style={{ width: `${pct}%` }} />
                      </div>
                      <span className="text-white/65 text-xs w-7 text-right">{pct}%</span>
                    </div>
                  ))}
                  <div className="mt-4 flex items-center gap-2 bg-success/20 rounded-lg px-3 py-2">
                    <RiCheckLine className="text-success shrink-0" />
                    <p className="text-success text-xs font-semibold">Excellent Match!</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-background">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-text-secondary text-lg">What our members say about RoomBridge</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TESTIMONIALS.map(({ name, city, role, rating, text }) => (
              <div key={name}
                ...
                <div className="flex mb-3" aria-label={`${rating} out of 5 stars`}>
                  {[...Array(rating)].map((_, i) => (
                    <RiStarFill key={i} className="text-warning text-sm" aria-hidden="true" />
                  ))}
                </div>
                <p className="text-text-secondary text-sm leading-relaxed mb-5 italic">"{text}"</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-text-secondary text-xs">{role} · {city}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-primary">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
            Ready to Find Your <span className="text-accent">Perfect Room?</span>
          </h2>
          <p className="text-white/70 text-lg mb-8">
            Join over 8,500 happy tenants who found their home through RoomBridge.
          </p>
