...
import {
  RiBuildingLine, RiGroupLine, RiShieldCheckLine,
  RiStarFill, RiArrowRightLine, RiHeartLine,
  RiHome4Line, RiSearchLine, RiMapPin2Line,
} from 'react-icons/ri';

/*
  BUG FIX: All 4 team entries had name 'Rahid Ali ' — identical names
  ...
  <div className="min-h-screen">
    ...
    <section className="bg-primary py-20 px-4 sm:px-6 lg:px-8 text-center relative overflow-hidden">
      ...
      <div className="relative max-w-3xl mx-auto">
        <div className="inline-flex items-center gap-2 bg-white/10 text-accent
                        px-4 py-1.5 rounded-full text-sm font-medium mb-5 border border-white/20">
          <RiBuildingLine /> Our Story
        </div>
        <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 leading-tight">
          Building Pakistan's Most <span className="text-accent">Trusted</span> Room Rental Platform
        </h1>
        <p className="text-white/70 text-lg max-w-xl mx-auto">
          We started RoomBridge because finding a safe, affordable room in Pakistan's cities was unnecessarily hard.
          We're changing that — city by city.
        </p>
      </div>
    </section>
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p className="text-text-secondary leading-relaxed mb-4">
            Every year, millions of Pakistanis move to new cities for study and work.
            The process of finding a safe, affordable room through classifieds is stressful, time-consuming, and risky.
          </p>
          <p className="text-text-secondary leading-relaxed mb-6">
            RoomBridge is on a mission to make room hunting as easy as booking a flight — with verified listings,
            smart roommate matching, and secure in-app communication.
          </p>
          <div className="flex flex-col gap-3">
            {['Verify every listing', 'Enable safe communication', 'Make room hunting effortless'].map((item) => (
              <div key={item} className="flex items-center gap-2 text-primary font-medium">
    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-primary mb-3">How RoomBridge Works</h2>
          <p className="text-text-secondary">Your journey from search to move-in</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { num: '1', icon: RiSearchLine, title: 'Search', desc: 'Use our filters to find rooms by city, budget, and amenities.' },
            { num: '2', icon: RiGroupLine, title: 'Match', desc: 'Get matched with compatible roommates using our smart algorithm.' },
            { num: '3', icon: RiHome4Line, title: 'Move In', desc: 'Confirm your booking, sign the agreement, and move in with confidence.' },
          ].map(({ num, icon: Icon, title, desc }) => (
            <div key={num} className="text-center p-6 bg-background rounded-card border border-border
                                      hover:shadow-hover transition-shadow duration-300">
              <div className="w-14 h-14 rounded-2xl bg-primary flex items-center justify-center mx-auto mb-4">
                <Icon className="text-white text-xl" />
              </div>
              <span className="text-xs font-bold text-accent tracking-widest block mb-2">STEP {num}</span>
          ...
          <div className="space-y-8">
            {MILESTONES.map(({ year, event }) => (
              <div key={year} className="flex items-start gap-5 pl-10 relative">
                <div className="absolute left-0 top-1 w-8 h-8 rounded-full bg-primary text-white
    ...
    <section className="py-20 px-4 sm:px-6 lg:px-8 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-primary mb-3">Meet the Team</h2>
          <p className="text-text-secondary">The people building Pakistan's housing future</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {TEAM.map(({ name, role, bio }) => (
            /* BUG FIX: key is now 'role' — guaranteed unique after fix above */
            <div key={role}
              ...
              <p className="text-secondary text-xs font-semibold mb-2">{role}</p>
              <p className="text-text-secondary text-xs leading-relaxed">{bio}</p>
            </div>
          ))}
        </div>
      </div>
