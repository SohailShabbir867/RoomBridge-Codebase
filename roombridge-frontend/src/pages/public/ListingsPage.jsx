...
import {
  RiFilterLine, RiSearchLine, RiMapPin2Line,
  RiHotelLine, RiGroupLine, RiWifiLine, RiTempColdLine,
  RiCarLine, RiCloseLine, RiArrowLeftLine, RiArrowRightLine,
  RiHeart3Line, RiHeart3Fill, RiGridLine, RiListCheck,
  RiSortAsc, RiHome4Line, RiLoader4Line, RiRefreshLine,
} from 'react-icons/ri';
...
/*
  BUG FIX: Budget filter was bound to filters.budget (a free string)
...
const AMENITY_ICONS = {
  WiFi: RiWifiLine, AC: RiTempColdLine, Parking: RiCarLine, default: RiHotelLine,
};
...
const ListingCard = ({ listing, view, onToggleSave }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to update saved listings.');
    } finally {
  ...
  return (
    <div className={`bg-white rounded-card border border-border shadow-card
                     ...
                     ${isGrid ? '' : 'flex gap-0'}`}>
      {/* Image */}
      <div className={`relative overflow-hidden ${isGrid ? 'w-full' : 'w-56 shrink-0'}`}>
        ...
        {/* Type badge */}
        <div className="absolute top-3 left-3 bg-primary/90 backdrop-blur-sm
      ...
      {/* Info */}
      <Link to={`/listings/${listing._id}`} className="block p-4 flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          {(listing.amenities || []).slice(0, 3).map((a) => {
            ...
            return <Icon key={a} className="text-text-secondary text-sm" title={a} aria-label={a} />;
          })}
          {listing.genderPreference && listing.genderPreference !== 'any' && (
            <span className="flex items-center gap-1 text-xs text-secondary font-medium">
              <RiGroupLine /> {listing.genderPreference === 'male' ? 'Male only' : 'Female only'}
            </span>
          )}
        </div>
        {/* Price — BUG FIX: listing.rent (not listing.price) */}
...
const ListingsPage = () => {
  ...
  /*
    BUG FIX: All filter keys now match the backend query param names.
    - 'type' → 'roomType'
  ...
  */
  ...
  const clearFilters = () => {
    setFilters({ city: '', roomType: '', budget: '', amenities: [], sortBy: 'newest', search: '' });
    setPage(1);
  };
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      /*
        BUG FIX: filter key is now 'roomType' (not 'type')
      */
      ...
      /*
        BUG FIX: 'budget' is a UI-only range string. Split into minRent/maxRent.
      */
      if (filters.budget) {
      ...
      */
      setListings(Array.isArray(res.listings) ? res.listings : Array.isArray(res.data) ? res.data : []);
  ...
  /*
    BUG FIX: 'filtered' was referenced as an undefined variable (ReferenceError crash).
    Replaced with the actual 'total' count from API response.
    BUG FIX: was checking filters.budget — now correctly checks filters.budget too.
  */
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4
          ...
          <div className="flex items-center gap-3 flex-1">
            ...
            >
              ...
              Filters
              {activeFilterCount > 0 && (
                <span className="ml-1 bg-accent text-primary text-xs font-bold
          ...
          <div className="flex items-center gap-3 shrink-0">
            <span className="text-sm text-text-secondary hidden sm:block">
              {loading ? '…' : `${total.toLocaleString()} room${total !== 1 ? 's' : ''} found`}
            </span>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex gap-6">
          ...
          {/* ── Filters sidebar ── */}
          {showFilters && (
            <aside className="w-64 shrink-0 hidden lg:block" aria-label="Listing filters">
              <div className="bg-white rounded-card border border-border p-5 sticky top-36">
                ...
                <div className="mb-5">
                  ...
                  <div className="relative">
                    ...
                    <select id="filter-city" value={filters.city}
                ...
                {/* Room Type
                  BUG FIX: radio buttons now bind to filters.roomType (not filters.type).
                  BUG FIX: ROOM_TYPES values now match backend enum.
                */}
                <div className="mb-5">
                    ...
                    {ROOM_TYPES.map(({ value, label }) => (
                      <label key={value} className="flex items-center gap-2 cursor-pointer group">
                        <input type="radio" name="roomType" value={value}
                               ...
                               onChange={() => handleFilter('roomType', filters.roomType === value ? '' : value)}
                ...
                {/* Budget
                  BUG FIX: was binding to filters.budget but sending it as a raw
                  string query param. Now just a UI selector; fetchListings splits it.
                */}
                <div className="mb-5">
                  <label className="label" htmlFor="filter-budget">Budget (PKR/month)</label>
                  <select id="filter-budget" value={filters.budget}
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-4">
              {/* BUG FIX: was referencing undefined 'filtered.length' — use total */}
              ...
              <div className="flex items-center gap-2 ml-auto">
                ...
                {/* BUG FIX: was reading filters.sort (undefined) — corrected to filters.sortBy */}
            ...
            {!error && !loading && listings.length === 0 && (
              <div className="text-center py-20">
                <RiHome4Line className="text-5xl text-border mx-auto mb-4" aria-hidden="true" />
                <h3 className="text-xl font-bold text-primary mb-2">No Rooms Found</h3>
            ...
            {!error && !loading && listings.length > 0 && (
              <div className={
                ...
                {listings.map((listing) => (
                  <ListingCard key={listing._id} listing={listing} view={view} />
                ))}
              </div>
            )}
