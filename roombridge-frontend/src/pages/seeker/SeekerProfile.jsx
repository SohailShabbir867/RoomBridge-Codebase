...
import { setCredentials, updateUser, logout } from '../../redux/slices/authSlice';
import authService from '../../services/authService';
import toast from 'react-hot-toast';
import {
  RiArrowLeftLine, RiUserLine, RiMailLine, RiPhoneLine,
  RiMapPin2Line, RiEditLine, RiCheckLine, RiLoader4Line,
  RiLogoutBoxLine, RiLockLine, RiImageAddLine,
} from 'react-icons/ri';
...
const SeekerProfile = () => {
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      ...
      /*
        BUG FIX: Old code called setCredentials({ user: ... }) which overwrites
        isAuthenticated. Use updateUser instead which merges at the slice level.
      */
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-20 bg-white border-b border-border px-6 py-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-text-secondary text-xs">Manage your account details</p>
        </div>
      </header>
        ...
        <div className="bg-white rounded-card border border-border shadow-card p-6">
          <div className="flex items-start gap-5">
            ...
            <div className="relative shrink-0">
              ...
              <label className="absolute -bottom-2 -right-2 w-7 h-7 bg-primary rounded-full flex items-center
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-3 space-y-1.5 text-sm text-text-secondary">
                <p className="flex items-center gap-2"><RiMailLine className="text-secondary" /> {user?.email}</p>
                {user?.phone && <p className="flex items-center gap-2"><RiPhoneLine className="text-secondary" /> {user.phone}</p>}
                {user?.city  && <p className="flex items-center gap-2"><RiMapPin2Line className="text-secondary" /> {user.city}</p>}
              </div>
            </div>
          </div>
          ...
          {/* Edit form */}
          {editing && (
            <form onSubmit={handleSaveProfile} className="mt-6 pt-6 border-t border-border space-y-4">
              <div>
          ...
          {showPwSection && (
            <form onSubmit={handleChangePassword} className="space-y-4">
              ...
              <div>
                ...
                {pwErrors.confirm && <p className="error-msg">{pwErrors.confirm}</p>}
              </div>
