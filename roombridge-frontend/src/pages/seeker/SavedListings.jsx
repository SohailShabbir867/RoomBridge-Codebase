...
import {
  RiArrowLeftLine, RiHeart3Fill, RiLoader4Line,
  RiHome4Line, RiMapPin2Line, RiSearchLine, RiArrowRightLine,
} from 'react-icons/ri';
...
const SavedListings = () => {
  ...
  useEffect(() => {
    listingService.getSavedListings()
      .then(res => {
        /*
          BUG FIX: Backend getSavedListings returns { success, listings, pagination }.
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-20 bg-white border-b border-border px-6 py-4 flex items-center gap-4">
        ...
        <div>
          <h1 className="font-bold text-primary">Saved Rooms</h1>
          <p className="text-text-secondary text-xs">{listings.length} saved listing{listings.length !== 1 ? 's' : ''}</p>
        </div>
      </header>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        {loading ? (
          ...
          <div className="text-center py-20 bg-white rounded-card border border-border shadow-card">
            ...
            <p className="text-primary font-semibold text-xl mb-2">No Saved Rooms Yet</p>
