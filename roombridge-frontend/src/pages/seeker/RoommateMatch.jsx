...
import {
  RiArrowLeftLine, RiGroupLine, RiLoader4Line,
  RiSaveLine, RiMapPin2Line, RiCheckLine,
  RiHeart3Line, RiHome4Line, RiArrowRightLine,
} from 'react-icons/ri';
...
const getScoreColor = (score) => {
  if (score >= 70) return { ring: 'stroke-success', text: 'text-success',  bg: 'bg-success/10'  };
...
const RoommateMatch = () => {
  ...
  useEffect(() => {
    api.get('/preferences/me')
      .then(res => {
        const p = res.data?.preference || res.data?.data || null;
        setPref(p);
        if (p) {
          setForm({
      ...
      .catch(() => { /* No preferences yet — form stays blank */ })
    ...
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-20 bg-white border-b border-border px-6 py-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-text-secondary text-xs">Find compatible roommates based on your lifestyle</p>
        </div>
      </header>
              ...
              <div className="lg:col-span-2">
                <div className="bg-white rounded-card border border-border shadow-card p-6">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                      <h2 className="font-semibold text-primary">Lifestyle Preferences</h2>
                      <p className="text-xs text-text-secondary">
                        {pref ? 'Update your preferences to refresh matches.' : 'Complete your profile to find matches.'}
                      </p>
                    </div>
                  </div>

                  <form onSubmit={handleSave} className="space-y-5">
                    <OptionRow label="Sleep Schedule *" name="sleepSchedule" options={SLEEP_OPTIONS} />
                    <OptionRow label="Occupation *"     name="occupation"    options={OCCUPATION_OPTIONS} />
                    <OptionRow label="Your Gender *"    name="gender"        options={GENDER_OPTIONS} />
                    <OptionRow label="Roommate Gender"  name="genderPreference" options={GENDER_PREF_OPTIONS} />

                    {/* Cleanliness slider */}
                    <div>
                      <p className="label mb-2">Cleanliness Level * ({form.cleanliness}/5)</p>
                      <input type="range" min={1} max={5} step={1} value={form.cleanliness}
                             onChange={e => setForm(f => ({ ...f, cleanliness: parseInt(e.target.value) }))}
                    ...
                    <label className="flex items-center gap-3 p-3 bg-background rounded-input border border-border cursor-pointer">
                      <input type="checkbox" checked={form.pets}
                             onChange={e => setForm(f => ({ ...f, pets: e.target.checked }))}
                             className="accent-primary w-4 h-4" />
                    ...
                    {/* Budget */}
                    <div>
                      <label className="label" htmlFor="rm-budget">Monthly Budget (PKR)</label>
                      <input id="rm-budget" type="number" min={1000} placeholder="e.g. 15000"
                             value={form.budget}
                    ...
                    {/* City */}
                    <div>
                      <label className="label" htmlFor="rm-city">Preferred City</label>
                      <select id="rm-city" value={form.preferredCity}
                    ...
                    {/* Bio */}
                    <div>
                      <label className="label" htmlFor="rm-bio">About Me</label>
                      <textarea id="rm-bio" value={form.bio}
                    ...
                    <button type="submit" disabled={prefSaving}
                      ...
                      {prefSaving
                        ...
                        : <><RiSaveLine /> {pref ? 'Update Preferences & Refresh Matches' : 'Save & Find Matches'}</>}
                    </button>
                  </form>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {pref && (
                  <div className="bg-white rounded-card border border-border shadow-card p-5">
                    <h3 className="font-semibold text-primary mb-3 text-sm">Your Current Profile</h3>
                    <div className="space-y-2">
                      ...
                      ].filter(x => x.val !== undefined && x.val !== null && x.val !== '').map(({ label, val }) => (
                    ...
                    <button onClick={() => { setTab('matches'); loadMatches(); }}
                ...
                <div className="bg-primary/5 rounded-card border border-primary/20 p-5">
                  <h3 className="font-semibold text-primary mb-2 text-sm">How Matching Works</h3>
        ...
        {tab === 'matches' && (
          !pref ? (
            <div className="text-center py-20 bg-white rounded-card border border-border shadow-card">
              ...
              <h3 className="text-xl font-bold text-primary mb-2">Set Up Your Preferences First</h3>
              <p className="text-text-secondary text-sm mb-5">
                Fill in your lifestyle preferences to see compatible roommate matches.
              </p>
              <button onClick={() => setTab('preferences')} className="btn-primary">
                Set Preferences
              </button>
            </div>
          ) : matchLoading ? (
            <div className="flex items-center justify-center py-20">
              <div className="text-center">
                ...
                <p className="text-text-secondary text-sm">Finding your best matches…</p>
              </div>
            </div>
          ) : matchError ? (
            <div className="text-center py-20 bg-white rounded-card border border-border shadow-card">
              <p className="text-error text-sm mb-4">{matchError}</p>
              <button onClick={loadMatches} className="btn-secondary">Try Again</button>
            </div>
          ) : matches.length === 0 ? (
            <div className="text-center py-20 bg-white rounded-card border border-border shadow-card">
              <RiGroupLine className="text-5xl text-border mx-auto mb-4" />
              <h3 className="text-xl font-bold text-primary mb-2">No Matches Yet</h3>
              <p className="text-text-secondary text-sm mb-5">
                No other seekers have set up preferences yet. Check back soon, or update yours.
              </p>
              <button onClick={() => setTab('preferences')} className="btn-secondary">
                Update Preferences
              </button>
            </div>
          ) : (
            <>
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-text-secondary text-sm">Sorted by compatibility score</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {matches.map((match, idx) => {
                  ...
                  const score  = Math.round(match.compatibility?.score || match.compatibilityScore || match.score || 0);
                  const { bg }  = getScoreColor(score);

                  return (
                    <div key={seeker?._id || idx}
                      ...
                      <div className="flex items-start gap-4 mb-4">
                        ...
                        <div className="shrink-0">
                          {seeker?.profilePhoto?.url ? (
                            <img src={seeker.profilePhoto.url} alt={seeker.name}
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-primary text-base">{seeker?.name || 'Seeker'}</p>
                          {seeker?.city && (
                            <p className="text-xs text-text-secondary flex items-center gap-1 mt-0.5">
                              <RiMapPin2Line className="text-accent" /> {seeker.city}
                            </p>
                          )}
                          {/* Badge for score */}
                          <div className={`inline-flex items-center gap-1.5 mt-2 px-2.5 py-0.5
                      ...
                      {/* Breakdown */}
                      {(match.compatibility?.breakdown || match.breakdown) && Object.keys(match.compatibility?.breakdown || match.breakdown || {}).length > 0 && (
                        <div className="border-t border-border pt-4 mb-4">
                          <p className="text-xs font-semibold text-text-secondary mb-2">Score Breakdown</p>
                          <div className="space-y-1.5">
                            {Object.entries(match.compatibility?.breakdown || match.breakdown || {}).slice(0, 4).map(([key, val]) => (
                              <div key={key} className="flex items-center gap-2">
                                ...
                                <span className={`text-xs font-medium w-8 text-right ${getScoreColor(val).text}`}>
                                  {Math.round(val)}
                                </span>
                              </div>
                            ))}
                      ...
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {[match.preference?.sleepSchedule, match.preference?.occupation, match.preference?.gender]
                          .filter(Boolean)
                          .map(tag => (
                            <span key={tag} className="text-xs px-2 py-0.5 bg-background border border-border
                      ...
                      {/* Message button */}
                      <Link
                        to={`/seeker/messages?owner=${seeker?._id || ''}`}
