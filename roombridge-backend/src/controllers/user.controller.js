...
/* ── Sensitive fields to always exclude from user responses ─
   BUG FIX: findByIdAndUpdate() can return the full document including
...
const getProfile = async (req, res, next) => {
  try {
    /* BUG FIX: Explicitly exclude sensitive fields.
...
const updateProfile = async (req, res, next) => {
  try {
    ...
    if (Object.keys(updates).length === 0) {
      return errorResponse(res, 400, `No valid fields provided. Allowed: ${ALLOWED.join(', ')}.`);
    }
    ...
    if (updates.name !== undefined) {
      ...
      if (name.length < 3 || name.length > 50) {
        return errorResponse(res, 400, 'Name must be between 3 and 50 characters.');
      }
      updates.name = name;
    }
    ...
    if (updates.phone !== undefined && updates.phone !== '') {
      ...
      if (!/^(\+92|0)[0-9]{10}$/.test(phone)) {
        return errorResponse(res, 400, 'Please provide a valid Pakistani phone number (e.g. 03001234567).');
      }
      updates.phone = phone;
    }
    ...
    /* BUG FIX: select() the sensitive fields OUT of the returned document.
...
const updateProfilePhoto = async (req, res, next) => {
  try {
    ...
    /* ── Upload new photo FIRST, THEN delete old one ────────
       BUG FIX: Original deleted old photo BEFORE uploading new one.
       If the upload fails, the user is left with NO profile photo.
       Fix: upload first, delete old only on success. */
    let newPhoto;
    try {
      newPhoto = await uploadToCloudinary(req.file.buffer, 'roombridge/avatars');
    } catch (uploadErr) {
    ...
    if (user.profilePhoto?.public_id) {
      deleteFromCloudinary(user.profilePhoto.public_id)
        .catch(err => console.error('[Cloudinary] Failed to delete old avatar:', err.message));
    }
...
const removeProfilePhoto = async (req, res, next) => {
  try {
    ...
    await deleteFromCloudinary(user.profilePhoto.public_id)
      .catch(err => console.error('[Cloudinary] Failed to delete avatar:', err.message));
...
const getAllSeekers = async (req, res, next) => {
  try {
    ...
    const ownerPreference = await Preference.findOne({ user: req.user._id }).lean();
    ...
    /* BUG FIX: N+1 query — original fetched user details inside a loop.
       Fetch ALL seeker preferences in ONE query, build a lookup map. */
    const seekerIds  = seekers.map(s => s._id);
    const seekerPrefs = await Preference.find({ user: { $in: seekerIds } }).lean();
    const prefMap = {};
    seekerPrefs.forEach(p => { prefMap[p.user.toString()] = p; });
    ...
    const enriched = seekers.map(seeker => {
      ...
      if (ownerPreference && seekerPref) {
        /* BUG FIX: calculateCompatibility() already returns label in result.
        ...
        try {
          ...
          console.warn(`[Compatibility] Failed for seeker ${seeker._id}:`, calcErr.message);
        }
      }
