...
/* ── Safe integer parse with fallback ──────────────────────
   BUG FIX: parseInt('abc') returns NaN; Math.max(1, NaN) → NaN.
...
/* ── Parse JSON field from multipart/form-data ─────────────
   BUG FIX: silently swallowed parse errors left an unparsed string in the
...
const getAllListings = async (req, res, next) => {
  try {
    const {
      ...
      furnished, search, page = 1, limit = 12, sortBy = 'newest',
    } = req.query;
    ...
    /* BUG FIX: genderPreference filter — when a seeker passes their gender
    ...
    /* BUG FIX: Text search — when $text is in the filter, sort MUST include
    ...
    /* ── Pagination ─────────────────────────────────────── */
    /* BUG FIX: safeInt instead of Math.max(1, parseInt(...))
    ...
    /* BUG FIX: Add isSaved flag per listing when user is authenticated.
    ...
    if (req.user) {
      ...
      enriched = listings.map(l => ({
        ...l,
        isSaved: Array.isArray(l.savedBy) && l.savedBy.some(id => id.toString() === userId),
      }));
    }
...
const getListingById = async (req, res, next) => {
  try {
    const listing = await Listing.findById(req.params.id)
      .populate('owner', 'name profilePhoto city phone email createdAt')
      .lean();
    ...
    /* BUG FIX: Removed the auto-increment from here.
    ...
    const isSaved = req.user
      ? (listing.savedBy || []).some(id => id.toString() === req.user._id.toString())
      : false;
  ...
  try {
    ...
    try {
      photos = await Promise.all(
        req.files.map(file => uploadToCloudinary(file.buffer, 'roombridge/listings'))
      );
    } catch (uploadErr) {
      return errorResponse(res, 502, `Photo upload failed: ${uploadErr.message}`);
    }

    const {
      title, description, rent, city, address, area,
      roomType, genderPreference, availableFrom, furnished,
      features, nearbyPlaces, roommatePreferences,
    } = req.body;

    /* BUG FIX: parseJSON returns [] / {} fallback on any parse failure
...
const updateListing = async (req, res, next) => {
  try {
    ...
    if (listing.status === 'pending') {
      return errorResponse(res, 400, 'This listing is currently under review and cannot be edited.');
    }

    const {
      title, description, rent, city, address, area,
      roomType, genderPreference, availableFrom, furnished,
      features, nearbyPlaces, roommatePreferences, removePhotos,
    } = req.body;

    /* ── Step 1: Remove old photos from Cloudinary FIRST ────
       BUG FIX: Photo count check was done BEFORE removing old photos.
    ...
    if (Array.isArray(toRemove) && toRemove.length > 0) {
      ...
      await Promise.allSettled(toRemove.map(pid => deleteFromCloudinary(pid)));
      /* Remove from in-memory array */
      listing.photos = listing.photos.filter(p => !toRemove.includes(p.public_id));
    }
    ...
    if (req.files && req.files.length > 0) {
      ...
      try {
        newPhotos = await Promise.all(
          req.files.map(f => uploadToCloudinary(f.buffer, 'roombridge/listings'))
        );
      } catch (uploadErr) {
        return errorResponse(res, 502, `Photo upload failed: ${uploadErr.message}`);
      }
    ...
    /* ── Update scalar fields ──────────────────────────────
       BUG FIX: use explicit undefined checks instead of truthiness.
    ...
    if (genderPreference !== undefined) listing.genderPreference = genderPreference;
    ...
    if (pFeatures !== null && Array.isArray(pFeatures)) listing.features            = pFeatures;
    if (pNearby   !== null && Array.isArray(pNearby))   listing.nearbyPlaces         = pNearby;
    if (pRoommate !== null && typeof pRoommate === 'object') listing.roommatePreferences = pRoommate;
...
const deleteListing = async (req, res, next) => {
  try {
    ...
    if (!isOwner && !isAdmin) {
      return errorResponse(res, 403, 'You do not have permission to delete this listing.');
    }
    ...
    /* ── Notify seekers with active bookings, then cancel ───
       BUG FIX: was using Booking.deleteMany — erased booking history.
    ...
    if (activeBookings.length > 0) {
      ...
      await Booking.updateMany(
        ...
        { $set: { status: 'cancelled', ownerNote: 'The listing was removed by the owner.' } }
      );
    }
...
const getOwnerListings = async (req, res, next) => {
  try {
    ...
    /* BUG FIX: validate status param before using in query */
...
const saveListing = async (req, res, next) => {
  try {
    ...
    /* BUG FIX: Use atomic findByIdAndUpdate with $addToSet instead of
    ...
    const listing = await Listing.findById(listingId).select('status savedBy owner title');
    ...
    if (alreadySaved) {
      ...
      return successResponse(res, 200, 'Listing already saved.', { isSaved: true });
    }

    /* Atomic updates on both documents */
    await Promise.all([
      Listing.findByIdAndUpdate(listingId, { $addToSet: { savedBy: userId } }),
      User.findByIdAndUpdate(userId,        { $addToSet: { savedListings: listingId } }),
    ]);

    return successResponse(res, 200, 'Listing saved successfully.', { isSaved: true });
  } catch (err) {
    next(err);
  }
};
...
const unsaveListing = async (req, res, next) => {
  try {
    ...
    /* BUG FIX: Use $pull on both documents atomically instead of loading
    ...
    return successResponse(res, 200, 'Listing unsaved successfully.', { isSaved: false });
  } catch (err) {
    next(err);
  }
};
...
const getSavedListings = async (req, res, next) => {
  try {
    ...
    const enriched = listings.map(l => ({ ...l, isSaved: true }));
...
/* ══════════════════════════════════════════════════════════
   ...
   BUG FIX: getListingById no longer auto-increments — this is the
...
const incrementViews = async (req, res, next) => {
  try {
    ...
    return successResponse(res, 200, 'View count updated.', { views: listing.views });
  } catch (err) {
    next(err);
  }
};
