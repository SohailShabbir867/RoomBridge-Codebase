...
/* ── Helper: build a safe user payload (NEVER include password) ──────────
   Centralise the "safe user" shape so all endpoints return consistent
...
const registerUser = async (req, res) => {
  ...
  /* Validate role at controller level as a safety net.
...
const loginUser = async (req, res) => {
  ...
  /* Update lastLogin timestamp
     ...
     user.save({ validateBeforeSave: false }) with password in memory
     could re-trigger password hashing in some edge cases.
     findByIdAndUpdate bypasses pre-save entirely — safer here. */
  await User.findByIdAndUpdate(user._id, { lastLogin: new Date() });

  /* Explicitly delete password from the in-memory user object before
...
══════════════════════════════════════════════════════════ */
/* No `protect` middleware on this route (handled in routes file).
...
const getMe = async (req, res) => {
  if (!req.user) {
    return successResponse(res, 200, "No active session.", {
      user: null,
      preference: null,
    });
  }
...
const forgotPassword = async (req, res) => {
  ...
  /* Banned/inactive users should NOT receive a reset link.
...
const resetPassword = async (req, res) => {
  /* Sanitise the token parameter before hashing.
  ...
  /* Don't allow banned users to reset their way back in */
  if (user.isBanned) {
    return errorResponse(
      res,
      403,
  ...
  /* Reactivate account on successful reset if it was inactive
...
const updatePassword = async (req, res) => {
  ...
  /* Check that current and new passwords differ BEFORE hitting the DB.
  ...
  if (!isMatch) {
    /* Use 401 here (not 400). 401 = authentication failure.
  ...
  /* Explicitly clear the password from memory after save.
