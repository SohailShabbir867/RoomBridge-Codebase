 ...
 *
 * BUG FIX: The original had no `next` parameter usage but Express requires
...
const errorHandler = (err, req, res, next) => { // eslint-disable-line no-unused-vars
  ...
  if (err.code === 11000) {
    ...
    return errorResponse(res, 409, `${cap} already exists. Please use a different value.`);
  }
  ...
  if (err.code === 'LIMIT_FILE_SIZE') {
    return errorResponse(res, 413, 'File too large. Maximum allowed size is 5 MB.');
  }
  ...
  // BUG FIX: must check this BEFORE the dev-expose block so that
